...
// export
let drawing = function ({links, level, result}) {
    ...
    let another = nodes.map(d => ({name: d['name']}))
    let orgNode = nodes.map(d => d.name)
    let anotherLinks = links.map(d => ({
