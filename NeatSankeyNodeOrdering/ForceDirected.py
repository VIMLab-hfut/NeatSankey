...
def defaultPositionCalc(lvl, level_html):
    ...
    for i in range(len(lvl)):
        layerLength = 0
        defaultNodePadding = 150 // len(lvl[i])
        for t in range(len(lvl[i])):
            defaultPosition[i].append({
                "node": lvl[i][t],
                "x": defaultLayerPadding * i + defaultLayerPadding / 2,
                "y": 2.5 * level_html[i][t]["size"] + layerLength,
                "deltaX": 0,
                "deltaY": 0
            })
            layerLength += 5 * level_html[i][t]["size"] + defaultNodePadding
...
def initBorder(border, layerNum, layerLength, position, level_html, i):
    ...
    gap = min([node["size"] for node in level_html[i]])

    for node in range(layerNum):
        minmax = dict()
        if node == 0:
            minmax['min'] = 0
            minmax['max'] = 5 * level_html[i][node]['size']
        else:
            minmax['min'] = position[i][node]['y'] - 2.5 * level_html[i][node]['size'] \
                if position[i][node]['y'] - 2.5 * level_html[i][node]['size'] > 0 else 0
            ...
            minmax['max'] = position[i][node]['y'] + 2.5 * level_html[i][node]['size'] \
                if position[i][node]['y'] + 2.5 * level_html[i][node]['size'] < layerLength else layerLength
        border[node] = minmax
