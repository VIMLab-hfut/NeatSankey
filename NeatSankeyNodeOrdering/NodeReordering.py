...
def objFunc(layerList, edgesCopy):
    value = 0
    ...
    for i in range(layerLength1):
        for j in range(i+1, layerLength1):
            for p in range(layerLength2):
                node2 = layerList[0][j]
                node3 = layerList[1][p]
                wj = float(edgesCopy.get((node2, node3), 0))
                for q in range(p+1, layerLength2):
                    node1 = layerList[0][i]
                    node4 = layerList[1][q]
                    wi = float(edgesCopy.get((node1, node4), 0))
                    if wi != 0 and wj != 0:
                        # deltaW = 0.01 if math.fabs(wi - wj) < 0.01 else math.fabs(wi - wj)
                        value += wi * wj # 1 / deltaW

    return value

def generateConstraints(layerList, edges, originalDataCopy):
    ...
    while temper > temperEnd:
        for curIter in tqdm(range(maxIter)):
            ans = (0, 0)
            preVal = objFunc(layerList, edgesCopy)
            minVal = float('inf')
            for i in range(layerLength):
                for j in range(i+1, layerLength):
                    layerList[1][i], layerList[1][j] = layerList[1][j], layerList[1][i]
                    val = objFunc(layerList, edgesCopy)
                    layerList[1][i], layerList[1][j] = layerList[1][j], layerList[1][i]
                    if val < minVal:
                        minVal = val
                        ans = (i, j)
            dE = minVal - preVal
            if dE < 0:
                layerList[1][ans[0]], layerList[1][ans[1]] = layerList[1][ans[1]], layerList[1][ans[0]]
            # else:
            #     if math.e**(-dE/temper) > random.random():
            #         layerList[1][ans[0]], layerList[1][ans[1]] = layerList[1][ans[1]], layerList[1][ans[0]]
        temper *= delta
...
def graphCrossMin(G_inv, Levels, edges, originalData):
    ...
    for layer1List in layer1Choices:
        ...
        # Iter += 1
        # 两层两层进行
        for i in tqdm(range(len(lvl) - 1)):
            # layer_list2 = barycenterDirectlyCalc([currentLayersList[-1], lvl[i + 1]], N, constraints=[])
            # currentLayersList.append(generateConstraints([currentLayersList[-1], layer_list2], edges, originalDataCopy))
            currentLayersList.append(generateConstraints([currentLayersList[-1], lvl[i + 1]], edges, originalDataCopy))
            # currentLayersList.append(barycenterDirectlyCalc([currentLayersList[-1], lvl[i + 1]], N, constraints=[]))
